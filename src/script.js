...
function Header() {
  ...
  function Main() {
    ...
    const onSize = React.useCallback(
      (id, size) => setSizes((sizes) => ({ ...sizes, [id]: size })),
      [setSizes]
    );

    const sumWidth = React.useMemo(
      () =>
        Object.keys(sizes).reduce((acc, id) => acc + sizes[id].width, 0),
      [sizes]
    );

    const sumHeight = React.useMemo(
      () =>
        Object.keys(sizes).reduce((acc, id) => acc + sizes[id].height, 0),
      [sizes]
    );
